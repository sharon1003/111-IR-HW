	...
	with open("stopwords", "r", encoding='utf-8') as fin:
		...
		for q_id, query_sentence in tqdm(query_dict.items()):
			...
			for doc_id, tf in doc_tf_idf.items():
				...
				bm25_score = BM25(que_tf[q_id], doc_tf[doc_id], query_vocab, doc_idf, doc_len[doc_id], doc_avg)
				tmp_cos[doc_id] = vsm_score + bm25_score

			sorted_value = sorted(tmp_cos.items(), key=lambda x: x[1], reverse = True)

			bm15_score[q_id] = sorted_value[:1500]

			for key, values in bm15_score.items():
				ans = key + ","
				for v in values:
					ans = ans + v[0] + ' '
				ans = ans.strip() + '\n'
